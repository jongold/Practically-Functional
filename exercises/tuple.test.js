...
// This allows you to carry information along in the _1 position. But how do we access it?

// Tuples have an extend method. This passes the entire Tuple in. Let's see it in action:
...
Tuple(cfg, app)
  .map(app => addRoutes(app))
  .extend(t => t._2.listen(t._1.port))
  // Tuple(cfg, 'listening on 3000') .We have access to the whole tuple,
  // though it only alters position _2
  .extract();
  // 'listening on 3000'
...
test('Ex1: extend tuple', (assert) => {
  ...
  const expected = [
    { projectId: 1, name: 'Learn Extend', priority: 1 },
    { projectId: 1, name: 'Learn Extract', priority: 2 },
  ];
  expect(result).toEqual(expected);
});


// Ex2: Either also has extend, but not extract (because extract must always
// return a good value and Left breaks that law)
// Using the intuition of extend (it passes the full type in, but only alters
// what map would), rewrite testSuite to remove chain/map in favor of extend
// =========================
...
test('Ex2: extend tuple', () => {
  const one = { a: true, b: true, c: true };
  const two = { a: true, b: true, c: true };
  expect(testSuite(one, two)).toBe('2 passed');
  const two_ = { a: true, b: false, c: true };
  expect(testSuite(one, two_)).toBe('failed {\"a\":true,\"b\":false,\"c\":true}');
});
